namespace Lpp.Dns.Data.Migrations
{
    ...
    public partial class UpdateTVFGetAssignedUserNotifications_FilterDeletedOrganizations : DbMigration
    {
        public override void Up()
        {
            Sql(@"ALTER FUNCTION [dbo].[GetAssignedUserNotifications]
                    ...
                    (
	                    ...
	                    Union
	                    Select Events.Name As [Event], EventID, Organizations.Name As [Description], 'Organization' As [Level] from OrganizationEvents Inner Join Events On Events.ID = EventID Inner Join Organizations on Organizations.ID = OrganizationID Where Organizations.IsDeleted = 0 AND SecurityGroupID IN (Select SecurityGroupID from SecurityGroupUsers where UserID = @UserID)
	                    Union
