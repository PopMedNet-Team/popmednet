namespace Lpp.Dns.Data.Migrations
{
    ...
    public partial class AddGetNotifications : DbMigration
    {
        public override void Up()
        {
            Sql(@"CREATE FUNCTION [dbo].[EventGlobal]
(	
...
RETURN 
(
	SELECT DISTINCT e.EventID FROM GlobalEvents e JOIN SecurityGroupUsers sgu ON e.SecurityGroupID = sgu.SecurityGroupID WHERE sgu.UserID = @UserID AND e.EventID = @EventID AND e.Allowed = 1 AND 
		NOT EXISTS(SELECT NULL FROM GlobalEvents ne WHERE ne.EventID = e.EventID AND ne.SecurityGroupID = e.SecurityGroupID AND ne.Allowed = 0)
)
");
            Sql(@"CREATE FUNCTION [dbo].[EventDataMarts] 
(	
	@UserID uniqueidentifier,
	@EventID uniqueidentifier
)
...
RETURN 
(
	SELECT DISTINCT DataMartID FROM DataMartEvents de JOIN SecurityGroupUsers sgu ON de.SecurityGroupID = sgu.SecurityGroupID WHERE sgu.UserID = @UserID AND de.EventID = @EventID AND de.Allowed = 1 AND 
		NOT EXISTS(SELECT NULL FROM DataMartEvents nde WHERE nde.EventID = de.EventID AND nde.DataMartID = de.DataMartID AND nde.SecurityGroupID = de.SecurityGroupID AND nde.Allowed = 0)
)
");

            Sql(@"CREATE FUNCTION [dbo].[EventGroups] 
(	
	@UserID uniqueidentifier,
	@EventID uniqueidentifier
)
...
RETURN 
(
	SELECT DISTINCT GroupID FROM GroupEvents e JOIN SecurityGroupUsers sgu ON e.SecurityGroupID = sgu.SecurityGroupID WHERE sgu.UserID = @UserID AND e.EventID = @EventID AND e.Allowed = 1 AND 
		NOT EXISTS(SELECT NULL FROM GroupEvents ne WHERE ne.EventID = e.EventID AND ne.GroupID = e.GroupID AND ne.SecurityGroupID = e.SecurityGroupID AND ne.Allowed = 0)
)");

            Sql(@"CREATE FUNCTION [dbo].[EventOrganizations] 
(	
	@UserID uniqueidentifier,
	@EventID uniqueidentifier
)
...
RETURN 
(
	SELECT DISTINCT OrganizationID FROM OrganizationEvents e JOIN SecurityGroupUsers sgu ON e.SecurityGroupID = sgu.SecurityGroupID WHERE sgu.UserID = @UserID AND e.EventID = @EventID AND e.Allowed = 1 AND 
		NOT EXISTS(SELECT NULL FROM OrganizationEvents ne WHERE ne.EventID = e.EventID AND ne.OrganizationID = e.OrganizationID AND ne.SecurityGroupID = e.SecurityGroupID AND ne.Allowed = 0)
)
");

            Sql(@"CREATE FUNCTION [dbo].[EventProjectDataMarts]
(	
	@UserID uniqueidentifier,
	@EventID uniqueidentifier
)
...
RETURN 
(
	SELECT DISTINCT e.ProjectID, e.DataMartID FROM ProjectDataMartEvents e JOIN SecurityGroupUsers sgu ON e.SecurityGroupID = sgu.SecurityGroupID WHERE sgu.UserID = @UserID AND e.EventID = @EventID AND e.Allowed = 1 AND 
		NOT EXISTS(SELECT NULL FROM ProjectDataMartEvents ne WHERE ne.EventID = e.EventID AND ne.ProjectID = e.ProjectID AND ne.DataMartID = e.DataMartID AND ne.SecurityGroupID = e.SecurityGroupID AND ne.Allowed = 0)
)
");

            Sql(@"CREATE FUNCTION [dbo].[EventProjects]
(	
	@UserID uniqueidentifier,
	@EventID uniqueidentifier
)
...
RETURN 
(
	SELECT DISTINCT e.ProjectID FROM ProjectEvents e JOIN SecurityGroupUsers sgu ON e.SecurityGroupID = sgu.SecurityGroupID WHERE sgu.UserID = @UserID AND e.EventID = @EventID AND e.Allowed = 1 AND 
		NOT EXISTS(SELECT NULL FROM ProjectEvents ne WHERE ne.EventID = e.EventID AND ne.ProjectID = e.ProjectID AND ne.SecurityGroupID = e.SecurityGroupID AND ne.Allowed = 0)
)
");
            Sql(@"CREATE FUNCTION [dbo].[EventRegistries]
(	
	@UserID uniqueidentifier,
	@EventID uniqueidentifier
)
...
RETURN 
(
	SELECT DISTINCT e.RegistryID FROM RegistryEvents e JOIN SecurityGroupUsers sgu ON e.SecurityGroupID = sgu.SecurityGroupID WHERE sgu.UserID = @UserID AND e.EventID = @EventID AND e.Allowed = 1 AND 
		NOT EXISTS(SELECT NULL FROM RegistryEvents ne WHERE ne.EventID = e.EventID AND ne.RegistryID = e.RegistryID AND ne.SecurityGroupID = e.SecurityGroupID AND ne.Allowed = 0)
)
");
            Sql(@"CREATE FUNCTION [dbo].[EventUsers]
(	
	@UserID uniqueidentifier,
	@EventID uniqueidentifier
)
...
RETURN 
(
	SELECT DISTINCT e.UserID FROM UserEvents e JOIN SecurityGroupUsers sgu ON e.SecurityGroupID = sgu.SecurityGroupID WHERE sgu.UserID = @UserID AND e.EventID = @EventID AND e.Allowed = 1 AND 
		NOT EXISTS(SELECT NULL FROM UserEvents ne WHERE ne.EventID = e.EventID AND ne.UserID = e.UserID AND ne.SecurityGroupID = e.SecurityGroupID AND ne.Allowed = 0)
)
");
            Sql(@"CREATE FUNCTION [dbo].[GetNotifications]
(
	@UserID uniqueidentifier
)
RETURNS 
...
BEGIN
	...
	INSERT INTO @notifications (TimeStamp, Event, Message)
	SELECT LogsUserRegistrationSubmitted.TimeStamp, 'User Registered', LogsUserRegistrationSubmitted.Description FROM LogsUserRegistrationSubmitted JOIN UserEventSubscriptions ON LogsUserRegistrationSubmitted.EventID = UserEventSubscriptions.EventID 
		WHERE
		UserEventSubscriptions.UserID = @UserID
		AND
		(
	...
	SELECT LogsResponseViewed.TimeStamp, 'Results Viewed', LogsResponseViewed.Description FROM LogsResponseViewed JOIN UserEventSubscriptions ON LogsResponseViewed.EventID = UserEventSubscriptions.EventID 
		WHERE
		UserEventSubscriptions.UserID = @UserID
		AND EXISTS(SELECT NULL FROM dbo.FilteredRequestList(@UserID) r JOIN RequestDataMarts ON r.ID = RequestDataMarts.RequestID JOIN RequestDataMartResponses ON RequestDataMarts.ID = RequestDataMartResponses.RequestDataMartID WHERE RequestDataMartResponses.ID = LogsResponseViewed.ResponseID)
		AND
		(
	...
	INSERT INTO @notifications (TimeStamp, Event, Message)
	SELECT LogsRequestStatusChange.TimeStamp, 'Routing Status Changed', LogsRequestStatusChange.Description FROM LogsRequestStatusChange JOIN UserEventSubscriptions ON LogsRequestStatusChange.EventID = UserEventSubscriptions.EventID 
		WHERE
		UserEventSubscriptions.UserID = @UserID
		AND EXISTS(SELECT NULL FROM dbo.FilteredRequestList(@UserID) r WHERE r.ID = LogsRequestStatusChange.RequestID)
		AND
		(
	...
	SELECT LogsRoutingStatusChange.TimeStamp, 'Routing Status Changed', LogsRoutingStatusChange.Description FROM LogsRoutingStatusChange JOIN UserEventSubscriptions ON LogsRoutingStatusChange.EventID = UserEventSubscriptions.EventID 
		WHERE
		UserEventSubscriptions.UserID = @UserID
		AND EXISTS(SELECT NULL FROM dbo.FilteredRequestList(@UserID) r JOIN RequestDataMarts ON r.ID = RequestDataMarts.RequestID WHERE RequestDataMarts.ID = LogsRoutingStatusChange.RequestDataMartID)
		AND
		(
	...
	SELECT LogsResultsReminder.TimeStamp, 'Results Reminder', LogsResultsReminder.Description FROM LogsResultsReminder JOIN UserEventSubscriptions ON LogsResultsReminder.EventID = UserEventSubscriptions.EventID 
		WHERE
		UserEventSubscriptions.UserID = @UserID
		AND EXISTS(SELECT NULL FROM dbo.FilteredRequestList(@UserID) r WHERE r.ID = LogsResultsReminder.RequestID)
		AND
		(
	...
	SELECT LogsSubmittedRequestAwaitsResponse.TimeStamp, 'Request Awaits Response', LogsSubmittedRequestAwaitsResponse.Description FROM LogsSubmittedRequestAwaitsResponse JOIN UserEventSubscriptions ON LogsSubmittedRequestAwaitsResponse.EventID = UserEventSubscriptions.EventID 
		WHERE
		UserEventSubscriptions.UserID = @UserID
		AND EXISTS(SELECT NULL FROM dbo.FilteredRequestList(@UserID) r WHERE r.ID = LogsSubmittedRequestAwaitsResponse.RequestID)
	...
	INSERT INTO @notifications (TimeStamp, Event, Message)
	SELECT LogsSubmittedRequestNeedsApproval.TimeStamp, 'Request Awaits Response', LogsSubmittedRequestNeedsApproval.Description FROM LogsSubmittedRequestNeedsApproval JOIN UserEventSubscriptions ON LogsSubmittedRequestNeedsApproval.EventID = UserEventSubscriptions.EventID 
		WHERE
		UserEventSubscriptions.UserID = @UserID
		AND EXISTS(SELECT NULL FROM dbo.FilteredRequestList(@UserID) r WHERE r.ID = LogsSubmittedRequestNeedsApproval.RequestID AND r.Status = 300)
		AND
		(
	...
	INSERT INTO @notifications (TimeStamp, Event, Message)
	SELECT LogsUploadedResultNeedsApproval.TimeStamp, 'Results Reminder', LogsUploadedResultNeedsApproval.Description FROM LogsUploadedResultNeedsApproval JOIN UserEventSubscriptions ON LogsUploadedResultNeedsApproval.EventID = UserEventSubscriptions.EventID 
		WHERE
		UserEventSubscriptions.UserID = @UserID
		AND EXISTS(SELECT NULL FROM dbo.FilteredResponseList(@UserID) r WHERE r.RequestDataMartID = LogsUploadedResultNeedsApproval.RequestDataMartID)
		AND
		(
			...
			OR
			EXISTS(SELECT NULL FROM dbo.EventProjects(@UserID, '688B0001-1572-41CA-8298-A22200CBD542') e JOIN Requests ON e.ProjectID = Requests.ProjectID JOIN RequestDataMarts ON Requests.ID = RequestDataMarts.RequestID  WHERE RequestDataMarts.ID = LogsUploadedResultNeedsApproval.RequestDataMartID)
			OR
