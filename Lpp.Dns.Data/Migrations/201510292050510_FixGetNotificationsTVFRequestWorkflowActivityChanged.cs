namespace Lpp.Dns.Data.Migrations
{
    ...
    public partial class FixGetNotificationsTVFRequestWorkflowActivityChanged : DbMigration
    {
...
BEGIN
	...
	--Request Workflow Activity Changed
	INSERT INTO @notifications (TimeStamp, Event, Message)
	SELECT LogsRequestWorkflowActivityChange.TimeStamp, 'Request Workflow Activity Changed', LogsRequestWorkflowActivityChange.Description FROM LogsRequestWorkflowActivityChange JOIN UserEventSubscriptions ON LogsRequestWorkflowActivityChange.EventID = UserEventSubscriptions.EventID
	WHERE
	UserEventSubscriptions.UserID = @UserID
	AND EXISTS(SELECT NULL FROM dbo.FilteredRequestListforEvent(@UserID, null) r WHERE r.ID = LogsRequestWorkflowActivityChange.RequestID)
	AND
	(
	EXISTS(SELECT NULL FROM dbo.EventOrganizations(@UserID, 'D2DC0001-43E9-477E-B60D-A3BE01550FA4') e JOIN Requests ON e.OrganizationID = Requests.OrganizationID WHERE Requests.ID = LogsRequestWorkflowActivityChange.RequestID)
	OR
	EXISTS(SELECT NULL FROM dbo.EventProjects(@UserID, 'D2DC0001-43E9-477E-B60D-A3BE01550FA4') e JOIN Requests ON e.ProjectID = Requests.ProjectId WHERE Requests.ID = LogsRequestWorkflowActivityChange.RequestID)
	)
