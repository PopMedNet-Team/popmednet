namespace Lpp.Dns.Data.Migrations
{
    ...
    public partial class FixPMNDEV3475 : DbMigration
    {
					...
					(
						...
						(
							...
							(
								...
								OR
								EXISTS(SELECT NULL FROM DataMarts dm WHERE dm.ID = rdm.DataMartID AND EXISTS(SELECT NULL FROM dbo.AclOrganizationsFiltered(@UserID, '0CABF382-93D3-4DAC-AA80-2DE500A5F945', dm.OrganizationID)))
							)
						)
					)
					AND
					(
						...
						(	
							...
							(
								...
								OR
								EXISTS(SELECT NULL FROM DataMarts dm WHERE dm.ID = rdm.DataMartID AND EXISTS(SELECT NULL FROM dbo.AclOrganizationsFiltered(@UserID, '0CABF382-93D3-4DAC-AA80-2DE500A5F945', dm.OrganizationID) a WHERE a.Allowed > 0))
							)
						)
					)
				)
					...
					(
						...
						(-- look to see if the submitter can skip approval for the datamart: A0F5B621-277A-417C-A862-801D7B9030A2
							(
								...
								OR
								EXISTS(SELECT NULL FROM DataMarts dm WHERE dm.ID = rdm.DataMartID AND EXISTS(SELECT NULL FROM dbo.AclOrganizationsFiltered(@UserID, 'A0F5B621-277A-417C-A862-801D7B9030A2', dm.OrganizationID)))
								OR
							...
							(
								...
								AND
								EXISTS(SELECT NULL FROM DataMarts dm WHERE dm.ID = rdm.DataMartID AND EXISTS(SELECT NULL FROM dbo.AclOrganizationsFiltered(@UserID, 'A0F5B621-277A-417C-A862-801D7B9030A2', dm.OrganizationID) a WHERE a.Allowed > 0))
								AND
