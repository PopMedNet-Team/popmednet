namespace Lpp.Dns.Data.Migrations
{
    ...
    public partial class AddRequestCommentChangeToGetNotifications : DbMigration
    {
        public override void Up()
        {
            DropForeignKey("dbo.LogsRequestCommentChange", "TaskID", "dbo.Tasks");
            DropIndex("dbo.LogsRequestCommentChange", new[] { "TaskID" });
            DropColumn("dbo.LogsRequestCommentChange", "TaskID");
...
BEGIN
	...
	INSERT INTO @notifications (TimeStamp, Event, Message)
	SELECT LogsRequestAssignmentChange.TimeStamp, 'Request Assignment Changed', LogsRequestAssignmentChange.Description FROM LogsRequestAssignmentChange JOIN UserEventSubscriptions ON LogsRequestAssignmentChange.EventID = UserEventSubscriptions.EventID
		WHERE
		UserEventSubscriptions.UserID = @UserID
		AND
		(
			EXISTS(SELECT NULL FROM dbo.EventProjects(@UserID, '2DFE0001-B98D-461D-A705-A3BE01411396') e JOIN Requests r ON e.ProjectID = r.ProjectId WHERE r.ID = LogsRequestAssignmentChange.RequestID)
			OR
			EXISTS(SELECT NULL FROM dbo.EventOrganizations(@UserID, '2DFE0001-B98D-461D-A705-A3BE01411396') e JOIN Requests r ON e.OrganizationID = r.OrganizationID WHERE r.ID = LogsRequestAssignmentChange.RequestID)
			OR
			EXISTS(SELECT NULL FROM dbo.EventUsers(@UserID, '2DFE0001-B98D-461D-A705-A3BE01411396') e WHERE e.UserID = LogsRequestAssignmentChange.UserID)
		)
	...
	JOIN Requests request ON LogsRequestDocumentChange.RequestID = request.ID
		...
		-- make sure user is an assignee
		AND EXISTS(SELECT NULL FROM RequestUsers ru WHERE ru.RequestID = LogsRequestDocumentChange.RequestID AND ru.UserID = @UserID)
		AND 
		(
	...
	JOIN Requests request ON comment.ItemID = request.ID
		...
		-- make sure user is an assignee
		AND EXISTS(SELECT NULL FROM RequestUsers ru WHERE ru.RequestID = comment.ItemID AND ru.UserID = @UserID)
		AND 
		(
			-- not associated with a workflow task
...
END");
        ...
        {
            AddColumn("dbo.LogsRequestCommentChange", "TaskID", c => c.Guid());
            CreateIndex("dbo.LogsRequestCommentChange", "TaskID");
            AddForeignKey("dbo.LogsRequestCommentChange", "TaskID", "dbo.Tasks", "ID");
