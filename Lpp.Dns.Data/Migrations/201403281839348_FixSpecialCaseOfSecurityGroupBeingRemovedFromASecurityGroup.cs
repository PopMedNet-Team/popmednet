namespace Lpp.Dns.Data.Migrations
{
    ...
    public partial class FixSpecialCaseOfSecurityGroupBeingRemovedFromASecurityGroup : DbMigration
    {
        public override void Up()
        {
            Sql(@"ALTER trigger [dbo].[SecurityMembership_Delete] on [dbo].[SecurityMembership] after delete as
	...
	IF(EXISTS(SELECT NULL FROM @alternativeRoutes AS a WHERE EXISTS(SELECT NULL FROM SecurityGroups WHERE SID = a.[Start]) AND EXISTS(SELECT NULL FROM SecurityGroups WHERE SID = a.[End])))
	BEGIN
