...
namespace Lpp.Dns.Portal.Root
{
    public class Global : Lpp.Mvc.LppMvcComposableApplication
    {
        ...
        protected void Application_Start(object sender, EventArgs e)
        {
            ...
            //using (var scope = CompositionScoping.OpenScope(TransactionScope.Id))
            //{
            //    scope.ComposeExportedValue<HttpContextBase>(new HttpContextWrapper(HttpContext.Current));
            //    var sech = scope.Get<ISecurityObjectHierarchyService<Lpp.Dns.Model.DnsDomain>>();
            //    //var uow = scope.Get<IUnitOfWork<DnsDomain>>();
            //    var plugins = scope.Get<IPluginService>();
            //    var audit = scope.Get<IAuditService<Lpp.Dns.Model.DnsDomain>>();                
            //    plugins.GetPluginRequestTypes().ForEach(x =>
            //        sech.SetObjectInheritanceParent(x.Value.RequestType.AsSecurityObject(), VirtualSecObjects.AllRequestTypes));

            //    var userObjectKind = new[] { Lpp.Dns.Data.User.ObjectKind };
            //    audit.AllEventKinds.Values.ForEach(x =>
            //        sech.SetObjectInheritanceParent(x.AsSecurityObject(),
            //            x.AppliesTo.ObjectKindsInOrder.SequenceEqual(userObjectKind) ?
            //            VirtualSecObjects.AllPersonalEvents : VirtualSecObjects.AllEvents));
            //    sech.SetObjectInheritanceParent(VirtualSecObjects.AllPersonalEvents, VirtualSecObjects.AllEvents);
