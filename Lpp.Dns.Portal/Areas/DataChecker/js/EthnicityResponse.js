        ...
        var ViewModel = /** @class */ (function () {
            function ViewModel(parameters) {
                ...
                self.buildCharts = function (rxData) {
                    ...
                    var overallPercentPieSrc = new DataChecker.ChartSource($.Enumerable.From(self.OverallMetrics).Select(function (x) { return [x.Hispanic_Display + ' ' + x.Percent.toFixed(2) + '%', x.Percent / 100]; }).ToArray());
                    ...
                    self.PartnersByCode.forEach(function (item) {
                        ...
                        var s4 = new DataChecker.ChartSource($.Enumerable.From(item.Partners).Select(function (x) { return [x.Partner, Global.Helpers.ToPercent(x.Count, item.Total)]; }).ToArray(), item.Hispanic_Display);
                        ...
                        var s5 = new DataChecker.ChartSource($.Enumerable.From(item.Partners).Select(function (x) { return [x.Partner + ' ' + Global.Helpers.ToPercent(x.Count, item.Total).toFixed(2) + '%', Global.Helpers.ToPercent(x.Count, item.Total) / 100]; }).ToArray(), item.Hispanic_Display);
                ...
                $.when($.get('/DataChecker/Ethnicity/GetTermValues?requestID=' + self.requestID().toString(), null, "script"), $.get('/DataChecker/Ethnicity/ProcessMetricsByResponse?responseID=' + self.responseID().toString(), null, "script")).then(function (termValues, metricResult) {
                    var rxHispanics = termValues[0];
                    ...
                    self.OverallMetrics = $.Enumerable.From(result.OverallMetrics).Where(function (x) { return $.inArray(x.Hispanic_Display, self.ToHispanicValues(rxHispanics)) > -1; }).ToArray();
                    self.CodesByPartner = result.CodesByPartner || [];
                    self.PartnersByCode = $.Enumerable.From(result.PartnersByCode).Where(function (x) { return $.inArray(x.Hispanic_Display, self.ToHispanicValues(rxHispanics)) > -1; }).ToArray();
            ...
            ViewModel.prototype.ToHispanicValues = function (rxTypes) {
                var rxValues = [];
                rxTypes.forEach(function (amtType) {
                    switch (amtType) {
                        case 0:
