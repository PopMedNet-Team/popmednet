...
$(function utilities(){
    ...
    function gate(condition: () => boolean, func: Function): any {
        var lastArgs = null;
        var lastThis = null;

        var result = <any>function () {
            if (condition()) return func.apply(this, arguments);
            lastArgs = arguments;
            lastThis = this;
        };
        result.release = function () {
            if (lastArgs && condition()) func.apply(lastThis, lastArgs);
        };

        return result;
    }

    function releaseGate(gate: any) {
