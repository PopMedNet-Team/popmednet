namespace Lpp.Dns.Data.Migrations
{
    ...
    public partial class UpdateTVFForFilteredRequestList2 : DbMigration
    {
				...
				(
					(
						...
						(
							...
							AND 
							(
								EXISTS(SELECT NULL FROM dbo.AclOrganizationsFiltered(@UserID, '0549F5C8-6C0E-4491-BE90-EE0F29652422', Requests.OrganizationID) INNER JOIN DataMarts dm ON dm.OrganizationID = Requests.OrganizationID AND dm.ID = rdm.DataMartID)
								OR
					...
					(
						...
						(
							...
							AND 
							(
								EXISTS(SELECT NULL FROM dbo.AclOrganizationsFiltered(@UserID, '0549F5C8-6C0E-4491-BE90-EE0F29652422', Requests.OrganizationID) a INNER JOIN DataMarts dm ON dm.OrganizationID = Requests.OrganizationID AND dm.ID = rdm.DataMartID WHERE a.Allowed > 0)
								OR
					...
					(
						...
						EXISTS(
							...
							(
								...
								OR
								EXISTS(SELECT NULL FROM dbo.AclOrganizationsFiltered(@UserID, '5D6DD388-7842-40A1-A27A-B9782A445E20', Requests.OrganizationID) INNER JOIN DataMarts dm ON dm.ID = rdm.DataMartID AND dm.OrganizationID = Requests.OrganizationID)
								OR
					...
					(
						...
						EXISTS(
							...
							(
								...
								OR
								EXISTS(SELECT NULL FROM dbo.AclOrganizationsFiltered(@UserID, '5D6DD388-7842-40A1-A27A-B9782A445E20', Requests.OrganizationID) a INNER JOIN DataMarts dm ON dm.ID = rdm.DataMartID AND dm.OrganizationID = Requests.OrganizationID WHERE a.Allowed > 0)
								OR
				...
				(
					(
						...
						EXISTS (
							...
							AND
							(
								EXISTS(SELECT NULL FROM dbo.AclOrganizationsFiltered(@UserID, '40DB7DE2-EEFA-4D31-B400-7E72AB34DE99', Requests.OrganizationID) INNER JOIN DataMarts dm ON dm.OrganizationID = Requests.OrganizationID AND dm.ID = rdm.DataMartID)
								OR
					...
					(
						...
						EXISTS (
							...
							AND
							(
								EXISTS(SELECT NULL FROM dbo.AclOrganizationsFiltered(@UserID, '40DB7DE2-EEFA-4D31-B400-7E72AB34DE99', Requests.OrganizationID) a INNER JOIN DataMarts dm ON dm.OrganizationID = Requests.OrganizationID AND dm.ID = rdm.DataMartID WHERE a.Allowed > 0)
								OR
					...
					(
						...
						EXISTS (
							...
							(
								...
								OR
								EXISTS(SELECT NULL FROM dbo.AclOrganizationsFiltered(@UserID, 'A58791B5-E8AF-48D0-B9CD-ED0B54E564E6', Requests.OrganizationID) INNER JOIN DataMarts dm ON dm.ID = rdm.DataMartID AND dm.OrganizationID = Requests.OrganizationID)
							)
						)
					)
					AND
					(
						...
						EXISTS (
							...
							(
								...
								OR
								EXISTS(SELECT NULL FROM dbo.AclOrganizationsFiltered(@UserID, 'A58791B5-E8AF-48D0-B9CD-ED0B54E564E6', Requests.OrganizationID) a INNER JOIN DataMarts dm ON dm.ID = rdm.DataMartID AND dm.OrganizationID = Requests.OrganizationID WHERE a.Allowed > 0)
							)
						)
					)
				)
	...
	(
		...
		(
			...
			(
				NOT Requests.SubmittedByID IS NULL AND Status >= 500
				AND
				--get the ones the user has the rights to approve regardless of if they need approval or not
				(
					(
				...
				(
					...
					AND rdm.QueryStatusTypeId <> 1 --not in draft state
					AND NOT 
					(-- look to see if the submitter can skip approval for the datamart		
						(
							EXISTS(SELECT NULL FROM dbo.AclDataMartsFiltered(Requests.SubmittedByID, 'A0F5B621-277A-417C-A862-801D7B9030A2', rdm.DataMartID))
							AND
							NOT EXISTS(SELECT NULL FROM dbo.AclDataMartsFiltered(Requests.SubmittedByID, 'A0F5B621-277A-417C-A862-801D7B9030A2', rdm.DataMartID) AS a WHERE  a.Allowed = 0)
						)
						OR
						(
							EXISTS(SELECT NULL FROM dbo.AclProjectDataMartsFiltered(Requests.SubmittedByID, 'A0F5B621-277A-417C-A862-801D7B9030A2', Requests.ProjectID, rdm.DataMartID))
							AND
							NOT EXISTS(SELECT NULL FROM dbo.AclProjectDataMartsFiltered(Requests.SubmittedByID, 'A0F5B621-277A-417C-A862-801D7B9030A2', Requests.ProjectID, rdm.DataMartID) AS a WHERE  a.Allowed = 0)
						)	
						OR
						(
							EXISTS(SELECT NULL FROM dbo.AclProjectsFiltered(Requests.SubmittedByID, 'A0F5B621-277A-417C-A862-801D7B9030A2', Requests.ProjectID))
