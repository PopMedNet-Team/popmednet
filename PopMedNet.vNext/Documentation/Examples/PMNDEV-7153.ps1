...
if($RequestDataMartsResponse.IsSuccessStatusCode)
{
    ...
    foreach($requestDatamart in $requestDatamarts)
    {     
        #Making the Request to Get the Response History   
        $historyResponse = $client.GetAsync('/Requests/GetResponseHistory?requestDataMartID='+ $requestDatamart.ID +'&requestID=' + $requestID).Result
        if($historyResponse.IsSuccessStatusCode){
            #Parse the Response to Get the Response Object
            $historyContentBody = $historyResponse.Content.ReadAsStringAsync().Result | ConvertFrom-Json 
            $history = $historyContentBody[0].results

            $history
        }
        else
        {
            $responseBody = $historyResponse.Content.ReadAsStringAsync().Result

            $errorMessage = [string]::Format("Status code {0}. Reason {1}. Server reported the following message: {2}.", $historyResponse.StatusCode, $historyResponse.ReasonPhrase, $responseBody)
