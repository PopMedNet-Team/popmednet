		...
		if (private_singleton_isRunning) {
			try {
				...
				i2b2.CRC.ctrlr.history.queryDeleteNoPrompt(self.QM.id);
					...
					private_singleton_isRunning = false;
					$('runBoxText').innerHTML = "Run Query";
					self.dispDIV.innerHTML += '<div style="clear:both; height:16px; line-height:16px; text-align:center; color:r#ff0000;">QUERY CANCELED</div>';
	...
	function private_startQuery() {
		...
		this.callbackQueryDef.callback = function(results) {
			if (results.error) {
				...
				// LPP: If there is a returnUrl, it is a response simulated by PMN.
				//      Stop the timer and return immediately.
				var returnUrl = results.refXML.getElementsByTagName('returnUrl')[0];
				if(returnUrl) {					
					private_singleton_isRunning = false;
					return;
				}
