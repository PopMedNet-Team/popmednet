i2b2.hive.communicatorFactory = function(cellCode){
	...
	retCommObj._doSendMsg = function(funcName, originName, parameters, callback, transportOptions){
		...
		if (sProxy_Url) {
			...
			// LPP: Insert PMN session token and service url so that that the CRC interceptor cell knows
			//      where to redirect.
			var qs = getQueryString();
			sMsgValues.proxy_info += '<sessionToken>\n' + qs['sessionToken'] + '</sessionToken>';
			sMsgValues.proxy_info += '<service>\n' + qs['service'] + '</service>';
			if (qs['isresponse'])
				sMsgValues.proxy_info += '<isresponse>\n' + qs['isresponse'] + '</isresponse>';
			if (qs['isrequest'])
				sMsgValues.proxy_info += '<isrequest>\n' + qs['isrequest'] + '</isrequest>';
		} else {
			sProxy_Url = sUrl;
		}
	   ...
	   i2b2.PM.model.IdleTimer.updateTimer();   
		...
		if (!xmlRecv) {
			...
			if (has_error || s.getAttribute('type') != 'DONE') {
				...
				console.error(transport.responseText);
			} else { // LPP: If an PMN redirect plugin
				var qs = getQueryString();
				returnUrl = xmlRecv.getElementsByTagName('returnUrl')[0];
				if (qs['isrequest'] && returnUrl != null) {
					window.location = returnUrl.getAttribute('url');
				}
			}
		}
		// attach the parse() function
