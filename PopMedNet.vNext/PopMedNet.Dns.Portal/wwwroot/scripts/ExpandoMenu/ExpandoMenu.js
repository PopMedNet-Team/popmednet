import { Helpers } from "../page/global.js";
$(() => {
    var expandos = $("section.ExpandoFrame");
    $.each(expandos, (count, item) => {
    ...
    maximizable.find("img#btnMaximize").click((e) => {
        var expando = $(e.target).parents("section.ExpandoFrame").first();
    ...
    help.find("img#btnHelp").click((e) => {
        var expando = $(e.target).parents("section.ExpandoFrame").first();
        var helpText = expando.find("aside.Help").html();
        Helpers.ShowAlert("Help", helpText);
    });
