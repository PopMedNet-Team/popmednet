...
export class ViewModel extends Global.PageViewModel {
    ...
    public User: ViewModels.UserViewModel;
    ...
    //Security
    public Security: SecurityAcl.AclEditViewModel<ViewModels.AclUserViewModel>;
    public UserAcls: KnockoutObservableArray<ViewModels.AclUserViewModel>;

    //Events
    public Events: EventsAcl.EventAclEditViewModel<ViewModels.UserEventViewModel>;
    public UserEvents: KnockoutObservableArray<ViewModels.UserEventViewModel>;

    //Subscriptions
    public Subscriptions: KnockoutObservableArray<ViewModels.UserEventSubscriptionViewModel>;
    ...
    public AssignedNotificationsList: KnockoutObservableArray<ViewModels.AssignedUserNotificationViewModel>;
    public dataSource: kendo.data.DataSource;
    //UI Events
    public RemoveSecurityGroup: (data: ViewModels.SecurityGroupViewModel) => void;

    //For temporarily stopping this.Active subscription callback
    public StopActiveCallback: boolean = false;

    public readonly CanChangeLogin: boolean;
    public readonly CanChangePassword: boolean;
    public readonly CanManageSecurity: boolean;
    public readonly CanManageNotifications: boolean;
    public readonly CanViewUser: boolean;
    public readonly CanDeleteUser: boolean;
    public readonly CanEditUser: boolean;

    public FrequenciesTranslation = Enums.FrequenciesTranslation;

    constructor(
        screenPermissions: any[],
        canApproveReject: boolean,
        user: Interfaces.IUserDTO,
        securityGroups: Interfaces.ISecurityGroupDTO[],
        userAcls: Interfaces.IAclUserDTO[],
        userEvents: Interfaces.IUserEventDTO[],
        subscribedEvents: Interfaces.IUserEventSubscriptionDTO[],
        ...
        subscribableEventsList: Interfaces.IEventDTO[],
        assignedNotificationsList: Interfaces.IAssignedUserNotificationDTO[],
        ...
        this.User.Active.subscribe((value) => {
            ...
            if (value) {
                if (typeof this.User.ID() === 'undefined' || this.User.ID() == null) {
                    ...
                    WebApi.Users.HasPassword(this.User.ID()).done((hasPassword: boolean[]) => {
            ...
            if (value) {
                ...
                Global.Helpers.ShowDialog("Deactivate User: " + this.User.FirstName() + " " + this.User.LastName(), "/users/deactivate", ["close"], 600, 400).done((reason: string) => {
                    if (reason != null) {
        ...
        this.Security = new SecurityAcl.AclEditViewModel<ViewModels.AclUserViewModel>(permissionList, securityGroupTree, this.UserAcls, [{ Field: "UserID", Value: this.User ? this.User.ID() : null }], ViewModels.AclUserViewModel);
        ...
        this.Events = new EventsAcl.EventAclEditViewModel<ViewModels.UserEventViewModel>(eventList, securityGroupTree, this.UserEvents, [{ Field: "UserID", Value: this.User.ID() }], ViewModels.UserEventViewModel);
        ...
        this.SecurityGroupSelected = (e: kendo.ui.TreeViewSelectEvent) => {
            let tree: kendo.ui.TreeView = $("#tvSecurityGroupSelector").data("kendoTreeView");
        ...
        //Events
        this.RemoveSecurityGroup = (data: ViewModels.SecurityGroupViewModel) => {
    ...
    public onActivate(e) {
        if (e != undefined && e.item.id == "tbAuthentication") {
            ...
            WebApi.Users.ListDistinctEnvironments(this.User.ID()).done((audits) => {
                ...
                WebApi.Users.GetSetting(environments.map((item: { Name: string, ID: string }) => { return "Users.Details." + item.ID + ":" + Global.User.ID })).done((settings: Interfaces.IUserSettingDTO[]) => {
                    let mainTable = $('<table class="panel-body table table-stripped table-bordered table-hover"></table>');
                    let tableBody = $('<tbody></tbody>')

                    for (let i = 0; i < environments.length; i++) {
                        let environment = <{Name:string, ID:string }>environments[i];
    ...
    public Save(data, e, showPrompt: boolean = true): JQueryDeferred<void> {
        ...
        WebApi.Users.InsertOrUpdate([user]).done((users) => {
            ...
            let userAcls = null, userEvents = null, userSecurityGroups = <Interfaces.IUpdateUserSecurityGroupsDTO> null;
            if (self.HasPermission(PMNPermissions.User.ManageSecurity)) {
                ...
                let uSecurityGroups = self.SecurityGroups().map((sd) => {
                    let sg: Interfaces.ISecurityGroupDTO = {
                        ID: sd.ID(),
                        Name: sd.Name(),
                        Path: sd.Path(),
                        Type: sd.Type(),
            ...
            if (self.HasPermission(PMNPermissions.User.ManageNotifications)) {
                userSubscriptions = self.Subscriptions().map((s) => {
                    let subscription: Interfaces.IUserEventSubscriptionDTO = {
                        EventID: s.EventID(),
            ...
            $.when<any>(
                userAcls == null ? null : WebApi.Security.UpdateUserPermissions(userAcls),
                userEvents == null ? null : WebApi.Events.UpdateUserEventPermissions(userEvents),
                userSecurityGroups == null ? null : WebApi.Users.UpdateSecurityGroups(userSecurityGroups),
                userSubscriptions == null ? null : WebApi.Users.UpdateSubscribedEvents(userSubscriptions)
            ).done(() => {
                if (showPrompt) {
    ...
    public TranslateSecurityGroupKind(kind: Enums.SecurityGroupKinds): string {
...
function init() {
    ...
    $.when<any>(
        id == null ? null : WebApi.Users.GetPermissions([id], defaultScreenPermissions),
        id == null ? null : WebApi.Users.Get(id),
        id == null ? null : WebApi.Security.GetUserPermissions(id),
        id == null ? null : WebApi.Events.GetUserEventPermissions(id),
        WebApi.Users.GetGlobalPermission(PMNPermissions.Organization.ApproveRejectRegistrations),
        WebApi.Organizations.List(),
        WebApi.Security.GetAvailableSecurityGroupTree(),
        WebApi.Security.GetPermissionsByLocation([Enums.PermissionAclTypes.Users]),
        id == Global.User.ID ? WebApi.Events.GetEventsByLocation([Enums.PermissionAclTypes.Users, Enums.PermissionAclTypes.UserProfile]) : WebApi.Events.GetEventsByLocation([Enums.PermissionAclTypes.Users]),
        id == null ? null : WebApi.Users.GetSubscribableEvents(id, null, null, "Name"),
        id == null ? null : WebApi.Users.GetAssignedNotifications(id)
    ).done((
        screenPermissions: any[],
        user,
        userAcls,
        ...
        let deferred = $.Deferred<void>();
        let subscribedEvents: Interfaces.IUserEventSubscriptionDTO[] = [];
        let securityGroups: Interfaces.ISecurityGroupDTO[] = [];
        let canApproveRejectOrgLevel: boolean = false; //Set in the deferred below

        deferred.done(() => {
            $(() => {
                let bindingControl = $("#Content");
                let vm = new ViewModel(screenPermissions,
                    canApproveRejectGlobally || canApproveRejectOrgLevel,
                    user,
                    securityGroups || [],
                    userAcls || [],
                    userEvents || [],
        ...
        $.when<any>(
            id && screenPermissions.indexOf(PMNPermissions.User.ManageSecurity.toLowerCase()) > -1 ? WebApi.Users.MemberOfSecurityGroups(id) : null,
            user.ID && (screenPermissions.indexOf(PMNPermissions.User.ManageNotifications.toLowerCase()) > -1 || screenPermissions.indexOf(PMNPermissions.User.ManageNotifications) > -1) ? WebApi.Users.GetSubscribedEvents(user.ID) : null,
            user.OrganizationID == null ? null : WebApi.Organizations.GetPermissions([user.OrganizationID], [PMNPermissions.Organization.ApproveRejectRegistrations])
...
export class EventViewModel extends ViewModels.EventViewModel {
    public Frequency: KnockoutComputed<Enums.Frequencies>;
    public FrequencyForMy: KnockoutComputed<Enums.Frequencies>;
    public SupportsMyFrequency: KnockoutObservable<Boolean>;

    constructor(vm: ViewModel, event: Interfaces.IEventDTO) {
        super(event);
        ...
        this.FrequencyForMy = ko.computed<Enums.Frequencies>({
            read: () => {
        ...
        this.Frequency = ko.computed<Enums.Frequencies>({
            read: () => {
                for (let j = 0; j < vm.Subscriptions().length; j++) {
                    let sub = vm.Subscriptions()[j];
                    if (sub.EventID() == this.ID())
