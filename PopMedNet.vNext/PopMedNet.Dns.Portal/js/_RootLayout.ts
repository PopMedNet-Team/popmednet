...
import { Users as WebUsers, Helpers as HelpersService } from './Lpp.Dns.WebApi.js';
$(() => {
    //Set all of the items with title tags to have qtip enabled
    let title = $('img[title!=""], span[title!=""]');
    ...
    HelpersService.RegisterFailMethod((e: JQueryXHR) => {
...
export namespace UserSettingHelper {
    ...
    export function GetSettings(key: Array<string>): JQueryDeferred<Dns.Interfaces.IUserSettingDTO[]> {
        ...
        WebUsers.GetSetting(key).done((results) => {
	...
	export function SetSetting(key: string, setting: string): JQueryDeferred<void> {
        ...
        return <any>WebUsers.SaveSetting({
            Key: key,
    ...
    export function ApplySettingsToGrid(grid: kendo.ui.Grid, key: string) {
        ...
        UserSettingHelper.GetSetting(key).done((setting) => {
            if (setting)
                Global.Helpers.SetGridFromSettings(grid, setting);

            Global.Helpers.WatchGridForChanges(grid, () => {
                UserSettingHelper.SetSetting(key, Global.Helpers.GetGridSettings(grid));
            });
...
export namespace PMNPermissions {
    export class Group {
