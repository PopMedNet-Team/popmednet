...
export class AclEditViewModel<T extends ViewModels.AclViewModel>
{
    ...
    constructor(permissions: Interfaces.IPermissionDTO[], securityGroupTree: Interfaces.ITreeItemDTO[], acls: KnockoutObservableArray<T>, targets: AclTargets[], aclType, identifier: string = null) {
        let self = this;
            ...
            Global.Helpers.ShowConfirm("Confirmation", "<p>Are you sure that you want to remove the selected security group?</p>").done(() => {
                ...
                self.SecurityGroups.forEach((sg, index) => {
                    if (sg.ID == self.SelectedSecurityGroup()) {
                        ...
                        let cboSecurityGroups: kendo.ui.DropDownList = $('#cboSecurityGroups' + self.Identifier()).data("kendoDropDownList");
    ...
    public SelectSecurityGroup() {
        ...
        Global.Helpers.ShowDialog("Add Security Group", "/security/SecurityGroupWindow", ["close"], 950, 550).done((result: Interfaces.ISecurityGroupDTO) => {
            ...
            let cboSecurityGroups: kendo.ui.DropDownList = $('#cboSecurityGroups' + this.Identifier()).data("kendoDropDownList");
    ...
    private SortItems(items: Interfaces.ITreeItemDTO[]) {
        items.sort((a, b) => {
            return a.Name < b.Name ? -1 : 1;
        });

        items.forEach((subitems: Interfaces.ITreeItemDTO) => {
...
export class PermissionListViewModel<T extends ViewModels.AclViewModel> {
    ...
    constructor(vm: AclEditViewModel<T>, permission: Interfaces.IPermissionDTO) {
        var self = this;
        this.VM = vm;
